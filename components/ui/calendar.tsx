...
import { DayPicker, Dropdown, useDayPicker, useNavigation } from "react-day-picker"
        ...
        Dropdown: (props) => {
          ...
          if (props.name === "months") {
            ...
            return <Select onValueChange={(newValue) => {
              ...
              <SelectTrigger className="min-w-20">{format(currentMonth, "MMM")}</SelectTrigger>
              <SelectContent>
                {selectItems.map((selectItem) => (
                  <SelectItem key={selectItem.label} value={selectItem.value}>{selectItem.label}</SelectItem>
            ...
            return <Select onValueChange={(newValue) => {
              ...
              <SelectTrigger className="min-w-20">{currentMonth.getFullYear()}</SelectTrigger>
              <SelectContent>
                {selectItems.map((selectItem) => (
                  <SelectItem key={selectItem.value} value={selectItem.value}>{selectItem.value}</SelectItem>
                ))}</SelectContent></Select>
          } else return null
        },
      }}
